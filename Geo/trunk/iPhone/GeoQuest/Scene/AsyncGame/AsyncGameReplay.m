...
@implementation AsyncGameReplay
...
-(id) initWithAsyncGameUILayer:(AsyncGameUI *)asyncUI {
    if ((self = [super init])) {
        ...
        // Setup Layers
        asyncGameUI = asyncUI;
        [asyncGameUI setAsyncReplayLayer:self];
...
-(void) checkRaceData {
    ...
    [challengeQuery findObjectsInBackgroundWithBlock:^(NSArray *challengeObjectArray, NSError *error) {
        ...
        if ([PPRD isEqualToString:@""] || [CPRD isEqualToString:@""]) {
            ...
            CCLOG(@"AsyncGameReplay: No replay available.");
            [self hideLayerAndObjects];
            [asyncGameUI showTerritoryLayer];
        } else {
            [self showLayerAndObjects];
            // Show replay of both players
            CCLOG(@"AsyncGameReplay: Show replay");
        }
    }];
}
    ...
    if (yDif < 0) {
        ...
        if ([playerRaceDataArray count] != 0) {
            ...
            if (playerRaceData.time < currentTime) {
                ...
                if ((playerVehicle.position.x + ((winSize.width - playerVehicle.contentSize.width)/ASYNC_GAME_SCORE_TO_WIN * playerRaceData.points)) > finishingPoint) {
                    playerVehicle.position = ccp(finishingPoint, playerVehicle.position.y);
                } else {
                    playerVehicle.position = ccp(playerVehicle.position.x + ((winSize.width - playerVehicle.contentSize.width)/ASYNC_GAME_SCORE_TO_WIN * playerRaceData.points), playerVehicle.position.y);
                }
        ...
        if ([challengerRaceDataArray count] != 0) {
            ...
            if (challengerRaceData.time < currentTime) {
                ...
                if ((challengerVehicle.position.x + ((winSize.width - challengerVehicle.contentSize.width)/ASYNC_GAME_SCORE_TO_WIN * challengerRaceData.points)) > finishingPoint) {
                    challengerVehicle.position = ccp(finishingPoint, challengerVehicle.position.y);
                } else {
                    challengerVehicle.position = ccp(challengerVehicle.position.x + ((winSize.width - challengerVehicle.contentSize.width)/ASYNC_GAME_SCORE_TO_WIN * challengerRaceData.points), challengerVehicle.position.y);
                }
        ...
        if ([playerReverseRaceDataArray count] != 0) {
            ...
            if (playerRaceData.time > currentTime) {
                if ((playerVehicle.position.x - ((winSize.width - playerVehicle.contentSize.width)/ASYNC_GAME_SCORE_TO_WIN * playerRaceData.points)) < startingPoint) {
        ...
        if ([challengerReverseRaceDataArray count] != 0) {
            ...
            if (challengerRaceData.time > currentTime) {
                if ((challengerVehicle.position.x - ((winSize.width - challengerVehicle.contentSize.width)/ASYNC_GAME_SCORE_TO_WIN * challengerRaceData.points)) < startingPoint) {
